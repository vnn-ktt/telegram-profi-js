...
bot.command("start", async (ctx) => {
  ...
  try {
    ...
    await ctx.reply(
      "Сейчас в базе данных бота " +
        questionManager.getQuestionsCountByGrade() +
        " вопросов!"
    );

    await ctx.reply("Выбери сложность вопроса в меню 👇", {
    ...
    try {
      ...
      if (questionManager.isClickQuestionType(questionData)) {
        questionData.options.forEach((option) => {
          ...
          inlineKeyboard.text(option.text!, JSON.stringify(data)).row();
        });
        ...
        inlineKeyboard.text("Узнать ответ", JSON.stringify(data));
      }
      await ctx.reply(questionData.text, { reply_markup: inlineKeyboard });
...
bot.on("callback_query:data", async (ctx) => {
  ...
  try {
    ...
    if (questionManager.isClickQuestionType(questionData)) {
      ...
      await ctx.reply(questionManager.getQuestionAnswer(grade, questionId), {
        parse_mode: "HTML",
      });
    }
